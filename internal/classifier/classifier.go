...
func (c *Classifier) Classify(f *models.FileMeta, dupGroup string) *models.Candidate {
	...
	// thumbs.db is a Windows thumbnail cache. Its ".db" extension would
	// otherwise trigger the database-protection rule below and the cache
	// rule would never be reached.
	isThumb := base == "thumbs.db"

	// 1. Important files are NEVER candidates (document tahap 5).
	if !isThumb && (importantExtensions[ext] || importantExtensions[base]) {
		return nil
	}
	...
	lastUse := f.ModTime
	if f.AccessTime.After(lastUse) {
		lastUse = f.AccessTime
	}
	daysSinceUse := now.Sub(lastUse).Hours() / 24
	if f.Size >= c.cfg.Thresholds.LargeFileBytes &&
		daysSinceUse >= float64(c.cfg.Thresholds.OldFileDays) {
		return &models.Candidate{
			Meta:       f,
			Label:      models.LabelReview,
