...
// sed_org returns the readable app segment (original case) below the root.
func sed_org(path, root string) string {
	...
	// second segment is a company/product partner only when it is a folder
	// (no file extension); otherwise keep the top-level folder alone.
	isFolder := len(segs) >= 2 && !strings.Contains(segs[1], ".")
	if len(segs) >= 2 && !knownSafeFolderNames[sed_word(segs[0])] && isFolder {
