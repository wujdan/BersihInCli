...
type ScanOptions struct {
	...
	AutoApprove bool
	NoDashboard bool
	// MinConfidence is the threshold for auto-selection in --yes mode
	// (default 0.9 when <= 0).
	MinConfidence float64
	// Progress, if set, is called periodically during scanning with the
	// number of files processed so far (for live progress in hosts/CLI).
...
func (a *App) Scan(ctx context.Context, opts ScanOptions) (*models.ScanReport, error) {
	...
	if opts.AutoApprove {
		fmt.Printf("Mode non-interaktif (--yes). Hanya kandidat confidence >= %.0f%% yang dipilih.\n",
			minConf*100)
	}
	...
	for f := range filesCh {
		...
		processed++
		if opts.Progress != nil && processed%500 == 0 {
...
// autoSelect picks candidates above a confidence threshold for --yes mode.
func autoSelect(report *models.ScanReport, minConf float64) map[string]bool {
