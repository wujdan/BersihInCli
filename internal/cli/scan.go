...
import (
	...
	"storage-optimizer/internal/disk"
...
Contoh:
  ...
  storage-optimizer scan --mode=quick --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			opts := app.ScanOptions{
				...
				Progress: func(files int64) {
					fmt.Fprintf(os.Stderr, "\r  Memindai... %d file", files)
				},
			}
			...
			if f.exportPath != "" {
				if err := exportReport(report, f.exportPath); err != nil {
					return err
				}
				fmt.Printf("  ✔ Laporan kandidat diekspor ke: %s\n", f.exportPath)
			}
	...
	cmd.Flags().Float64Var(&f.minConfidence, "min-confidence", 0.9, "ambang confidence untuk auto-pilih (0-1, dipakai bersama --yes)")
	cmd.Flags().StringVar(&f.exportPath, "export", "", "ekspor daftar kandidat ke file (.csv, .tsv, atau .json)")
	return cmd
}
...
func printScanSummary(r *models.ScanReport) {
	...
	if len(r.Roots) > 0 {
		if free, total, err := disk.FreeSizeBytes(r.Roots[0]); err == nil {
			fmt.Printf("  Kapasitas    : %s tersisa dari %s\n",
				models.HumanBytes(free), models.HumanBytes(total))
		}
	}
	fmt.Println("  --------------------------------------------------------------------------")
	if len(r.Errors) > 0 {
